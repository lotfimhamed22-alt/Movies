...
import 'package:movies/core/constants/key_constatnt.dart';
...
class AddmovieCubit extends Cubit<AddmovieState> {
  ...
  void loadBookMark(int movieId) {
    isBookMark =
        getIt<CachHelper>().getData(key: KeyConstatnt.bookMark, id: movieId) ??
        true;
  ...
  void addMovie(MovieModelWatchlist movieToAdded) {
    ...
    try {
      ...
      bool isExist = movies.any((movie) => movie.id == movieToAdded.id);

      if (!isExist) {
        movies.add(movieToAdded);
